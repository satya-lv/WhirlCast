...
import { Download, CheckCircle, ChevronDown, ChevronRight } from 'lucide-react';
...
const CAT_MAP = {
  'REF_190L_DirectCool':'Direct Cool Ref', 'REF_240L_FrostFree':'Frost Free Ref',
  'REF_340L_TripleDoor':'Frost Free Ref',  'WM_7KG_TopLoad':'Washing Machine',
...
const INDIA_ROW = { name:'All India', vals:[18450,22800,26400,19200,17600,20400], total:124850 };

const CATEGORY_DATA = [
  { name:'Air Conditioner', vals:[5900,7300,8800,5800,4200,6200], total:38200, acc:'85.2', bias:'4.1' },
  { name:'Direct Cool Ref', vals:[4200,5100,6200,4800,4600,5150], total:30050, acc:'88.4', bias:'3.2' },
  { name:'Frost Free Ref',  vals:[2800,3400,4100,3200,3100,3500], total:20100, acc:'86.7', bias:'5.1' },
  { name:'Washing Machine', vals:[3800,4600,5400,3900,3800,4500], total:26000, acc:'89.1', bias:'2.8' },
  { name:'Microwave',       vals:[1200,1500,1600,1200,1400,1500], total:8400,  acc:'82.3', bias:'6.4' },
  { name:'Induction',       vals:[550, 700, 700, 500, 500, 550],  total:4100,  acc:'80.1', bias:'7.2' },
];

const BRANCH_DATA = [
  { name:'Mumbai',    vals:[2123,2486,3344,2180,1980,2450], total:14563, acc:'91.0', bias:'2.3' },
  { name:'New Delhi', vals:[2020,2387,3341,2050,1900,2302], total:14000, acc:'85.1', bias:'6.9' },
  { name:'Kolkata',   vals:[1562,2255,2887,1800,1620,2076], total:12200, acc:'81.0', bias:'8.2' },
  { name:'Chennai',   vals:[1972,2124,3097,1920,1780,2107], total:13000, acc:'79.4', bias:'9.1' },
  { name:'Bangalore', vals:[1578,1934,2791,1680,1530,1887], total:11400, acc:'89.2', bias:'3.4' },
  { name:'Hyderabad', vals:[1602,1995,2929,1740,1620,1914], total:11800, acc:'83.5', bias:'5.8' },
  { name:'Pune',      vals:[1432,1791,2651,1560,1450,1716], total:10600, acc:'88.0', bias:'3.8' },
  { name:'Ahmedabad', vals:[1532,1864,2666,1670,1520,1848], total:11100, acc:'86.2', bias:'4.5' },
];
...
export default function ForecastingReport() {
  ...
  useEffect(() => {
    fetch('/api/report').then(r => r.json()).then(setData).catch(console.error);
  }, []);
  ...
  const handleSignOff = async () => {
    ...
    setSignedOff(true); setSigningOff(false); setShowSignoffModal(false);
    toast.success('✅ May 2026 cycle closed and signed off');
  };
  ...
  /* Branch × SKU (live API data, unchanged) */
  const ffData = data.futureForecast || [];
  const byBranchSku = {};
  ffData.forEach(r => {
  ...
  const canSignOff = user?.role === 'demand_planning';
  ...
  const displayBranchRows = user?.role === 'branch_sales'
    ? BRANCH_DATA.filter(r => r.name === (user.branch || 'Mumbai'))
    : BRANCH_DATA;

  /* Expandable row renderer */
  const ExpandableRow = ({ row, ri, subRows, showAcc = true }) => {
  ...
  return (
    <div style={{ padding: isMobile ? '16px' : '24px', maxWidth:1400, margin:'0 auto', background:'var(--bg)', minHeight:'calc(100vh - 52px)', paddingBottom: isMobile ? 80 : undefined }}>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12, minWidth:700 }}>
              ...
              <tbody>
                <tr style={{ background:'var(--card)' }}
                  ...
                  <td style={{ ...tdStyle, textAlign:'left', fontWeight:700 }}>{INDIA_ROW.name}</td>
                  {INDIA_ROW.vals.map((v,i) => <td key={i} style={tdStyle}>{v.toLocaleString('en-IN')}</td>)}
                  <td style={{ ...tdStyle, fontWeight:700, background:'#EFF3FF', color:'var(--navy-accent)' }}>{INDIA_ROW.total.toLocaleString('en-IN')}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        {viewMode === 'category' && (
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12, minWidth:780 }}>
              ...
              <tbody>
                {CATEGORY_DATA.map((row, ri) => (
                  <ExpandableRow key={ri} row={row} ri={ri} subRows={expandedRow===row.name ? getCatBranchRows(row.name) : []}/>
                ))}
              </tbody>
            </table>
          </div>
      ...
      {/* Sign-off — demand_planning only */}
      {canSignOff && (
        <div>
          <div style={{ background:'#FFFBEB', border:'0.5px solid #FCD34D', borderRadius:10, padding:'14px 16px', marginBottom:16, fontSize:13, color:'#92400E' }}>
            ℹ️ This report reflects the finalized forecast after all overrides and demand sensing adjustments.
          </div>
          <button onClick={()=>setShowSignoffModal(true)} disabled={signedOff} style={{
            background:signedOff?'#16A34A':'var(--navy-accent)', color:'white', border:'none', borderRadius:10,
            padding:'14px 24px', fontSize:14, fontWeight:600, cursor:signedOff?'default':'pointer',
            display:'flex', alignItems:'center', gap:8, width:'100%', justifyContent:'center',
          }}>
            <CheckCircle size={16}/>
            {signedOff?'✅ Cycle Signed Off Successfully':'📋 Submit for Sign-off'}
          </button>
        </div>
      )}

      <Modal isOpen={showSignoffModal} onClose={()=>setShowSignoffModal(false)} title="May 2026 Forecast Cycle — Sign-off">
        <div>
          <div style={{ background:'var(--bg)', borderRadius:10, padding:16, marginBottom:16 }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:10, fontSize:12 }}>
              {[
                ['Cycle Duration','14-May to 15-May-2026'],['Scenarios Generated','2 | Finalized: Scenario 1'],
                ['Branch Overrides','6 of 8 submitted'],   ['Conflicts Resolved','3 of 3'],
                ['Demand Sensing','1 document applied (4 signals)'],['Total Forecasted Units','1,24,850'],
                ['Predicted Revenue','₹148.2 Crore'],      ['Accuracy','87.3%'],
              ].map(([k,v],i)=>(
                <div key={i}>
                  <div style={{ color:'var(--text-3)', fontSize:11, marginBottom:2 }}>{k}</div>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={()=>setShowSignoffModal(false)} style={{ flex:1, background:'var(--bg)', border:'0.5px solid var(--border)', borderRadius:8, padding:10, cursor:'pointer' }}>Cancel</button>
            <button onClick={handleSignOff} disabled={signingOff} style={{ flex:2, background:'#16A34A', color:'white', border:'none', borderRadius:8, padding:10, cursor:'pointer', fontWeight:600, display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
              <CheckCircle size={14}/>
              {signingOff?'Submitting…':'✅ Submit & Close Cycle'}
            </button>
          </div>
        </div>
      </Modal>
