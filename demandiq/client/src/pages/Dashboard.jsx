  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: 'calc(100vh - 52px)', padding: isMobile ? '16px' : '24px', paddingBottom: isMobile ? 80 : undefined }}>
      ...
      <div className="fade-up-2" style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
        ...
        <KPICard label="Pending Overrides"     value={liveSummary ? `${liveSummary.total_branches - liveSummary.branches_submitted} branches` : kpis ? `${kpis.pendingBranches} branches` : '5 branches'} badge={liveSummary ? `${liveSummary.branches_submitted} of 8 submitted` : 'Due 20-Jun-2026'} badgeType="warn" accentColor="#F59E0B" icon="⏳"/>
        <KPICard label="Unresolved Conflicts"  value={liveSummary ? `${liveSummary.conflicts_pending}` : '1'}    badge={liveSummary ? (liveSummary.conflicts_pending === 0 ? 'All clear' : `${liveSummary.conflicts_pending} flagged`) : 'Loading...'} badgeType={liveSummary?.conflicts_pending === 0 ? 'up' : 'warn'} accentColor="#EF4444" icon="⚠️"/>
      </div>
